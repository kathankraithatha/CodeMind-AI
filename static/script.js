document.addEventListener('DOMContentLoaded', () => {
    ...
    async function executeAnalysis(actionOverride = null) {
        ...
        if (!code.trim()) {
            targetContainer.innerHTML = '<div style="color: #ef4444; text-align: center; margin-top: 2rem;">Paste some code first.</div>';
            isGenerating = false;
            return;
        }
        ...
        try {
            if (isVisualizing) {
                ...
                const match = currentDiagramRaw.match(/data:image\/jpeg;base64,([a-zA-Z0-9+/=]+)/);
                if (match) {
                ...
                if (regenerateAnalysisBtn) regenerateAnalysisBtn.style.display = 'inline-flex';
            } else {
    ...
    if (generateDiagramBtn) generateDiagramBtn.addEventListener('click', () => executeAnalysis('Visualize Architecture'));
    if (regenerateDiagramBtn) regenerateDiagramBtn.addEventListener('click', () => executeAnalysis('Visualize Architecture'));
    if (regenerateAnalysisBtn) regenerateAnalysisBtn.addEventListener('click', () => executeAnalysis(actionSelect.value));
    ...
    function switchTab(tabId) {
        ...
        if(analysisContainer) analysisContainer.style.display = tabId === 'output' ? 'flex' : 'none';
        if(diagramContainer) diagramContainer.style.display = tabId === 'diagram' ? 'flex' : 'none';
        
        if (tabId === 'raw') {
            rawOutput.style.display = 'block';
            rawOutput.value = lastActiveTab === 'diagram' ? currentDiagramRaw : currentAnalysisRaw;
        } else {
            rawOutput.style.display = 'none';
        }

        if(tabOutputBtn) tabOutputBtn.style.color = tabId === 'output' ? 'var(--hl-color)' : 'var(--text-muted)';
        if(tabDiagramBtn) tabDiagramBtn.style.color = tabId === 'diagram' ? 'var(--hl-color)' : 'var(--text-muted)';
        if(tabRawBtn) tabRawBtn.style.color = tabId === 'raw' ? 'var(--hl-color)' : 'var(--text-muted)';
    }

    if(tabOutputBtn) tabOutputBtn.addEventListener('click', () => switchTab('output'));
    if(tabDiagramBtn) tabDiagramBtn.addEventListener('click', () => switchTab('diagram'));
    ...
    copyBtn.addEventListener('click', () => {
        let textToCopy = rawOutput.style.display === 'block' ? rawOutput.value : (lastActiveTab === 'diagram' ? currentDiagramRaw : currentAnalysisRaw);
    ...
    async function sendChat() {
        ...
        loadingMsg.innerHTML = '<span class="loading-dots">Thinking<span>.</span><span>.</span><span>.</span></span>';
